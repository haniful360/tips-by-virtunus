...
const OverDueTask = () => {
    ...
    return (
        <div>
            <div>
                <span className={`${montserrat.className} text-[16px] font-medium`}>Select Date:</span> <DatePicker className="rounded-lg text-[16px] hover:shadow-lg hover:bg-white bg-[#F1F1F2] border-2 border-[#D1E9FF] focus:outline-none transition duration-300" selected={startDate} dateFormat="Pp" onChange={(date) => setStartDate(date)} />
            </div>
                ...
                <div key={task.id}>
                    <div className={`${taskStates[index] ? 'rounded-t-[10px] bg-[#FFFFFF] p-[25px] mt-5 box-shadow relative' : 'bg-[#FFFFFF] p-[25px] rounded-[10px] mt-5 box-shadow relative'}`}>
                        <div className="lg:flex items-center gap-4 lg:w-[780px] lg:h-[85px] ">
                            <div className="">
                                <input onClick={() => document.getElementById('my_modal_6').showModal()} type="checkbox" className='w-6 h-4' />
